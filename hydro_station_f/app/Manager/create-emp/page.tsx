...
export default function CreateEmp() {
  const [state, action] = useActionState(createEmp, {
    error: "",
    message: "",
    data: {
      first_name: "",
      ...
      >
        ...
        <div className="p-3 w-full">
          <div className="grid grid-cols-[auto_1fr] gap-4 items-center">
            <button
              type="submit"
              className="border-2 px-4 py-2 bg-blue-400 rounded hover:bg-blue-700"
            >
              Register Employee
            </button>

            {state.error && (
              <div className="text-red-500 text-sm font-medium break-words">
                {state.error}
              </div>
            )}
          </div>
